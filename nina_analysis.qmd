...
# Wrapper functions to create our topgo objects
create_topgo_object <- function(res_df, go_terms_df = go_terms) {
  res_df <- merge_and_dropNas(res_df, go_terms_df)
  # We need sort the object by p-value for topgo to work properly
  temp_sorted <- res_df[order(res_df$padj),]
  # We now need to convert to topGOs genelist format
  temp_top_go <- temp_sorted$padj
  # We now need to convert to topGOs genelist format
  names(temp_top_go) <- temp_sorted$gene_id
  return(temp_top_go)
}

# We now need to convert to topGOs genelist format
topgo_Cp24 <- create_topgo_object(resC24_df)
topgo_Cp48 <- create_topgo_object(resC48_df)
topgo_Np24 <- create_topgo_object(resN24_df)
topgo_Np48 <- create_topgo_object(resN48_df)
...
```{r}
temp_filter <- function(res_df, logfold) {
  if (logfold > 0) {
    res_df <- res_df %>%
      filter(log2FoldChange >= logfold)
  } else if (logfold < 0) {
    res_df <- res_df %>%
      filter(log2FoldChange <= logfold)
  } else {
...
# filter step
up_Cp24 <- temp_filter(resC24_df, 1.5)
down_Cp24 <- temp_filter(resC24_df, -1.5)

up_Cp48 <- temp_filter(resC48_df, 1.5)
down_Cp48 <- temp_filter(resC48_df, -1.5)

up_Np24 <- temp_filter(resN24_df, 1.5)
down_Np24 <- temp_filter(resN24_df, -1.5)

up_Np48 <- temp_filter(resN48_df, 1.5)
down_Np48 <- temp_filter(resN48_df, -1.5)

# Merge go terms and drop na values
topgo_up_Cp24 <- create_topgo_object(up_Cp24, go_terms)
topgo_down_Cp24 <- create_topgo_object(down_Cp24, go_terms)

topgo_up_Cp48 <- create_topgo_object(up_Cp48, go_terms)
topgo_down_Cp48 <- create_topgo_object(down_Cp48, go_terms)

topgo_up_Np24 <- create_topgo_object(up_Np24, go_terms)
topgo_down_Np24 <- create_topgo_object(down_Np24, go_terms)

topgo_up_Np48 <- create_topgo_object(up_Np48, go_terms)
topgo_down_Np48 <- create_topgo_object(down_Np48, go_terms)
```
...
get_enrich_table <- function(go_obj, fisher_result, total_genes = NULL) {
  tab <- GenTable(go_obj,
    ...
    mutate(
      ...
      # We create a factor and remove dups then reverse for the order
...
# Wrapper function to eliminate boiler plate as much as possible
enrich_topgo_to_table <- function(topgo_object) {
  topgo_result <- go_bp(topgo_object)
  fisher_result <- elim_fisher(topgo_result)
  results_table <- get_enrich_table(topgo_result, fisher_result)
  return(results_table)
}
```

```{r}
up_Cp24_go_bp <- go_bp(topgo_up_Cp24)
down_Cp24_go_bp <- go_bp(topgo_down_Cp24)

up_Cp48_go_bp <- go_bp(topgo_up_Cp48)
down_Cp48_go_bp <- go_bp(topgo_down_Cp48)

up_Np24_go_bp <- go_bp(topgo_up_Np24)
down_Np24_go_bp <- go_bp(topgo_down_Np24)

up_Np48_go_bp <- go_bp(topgo_up_Np48)
down_Np48_go_bp <- go_bp(topgo_down_Np48)
```

```{r}
# Biological Process (BP)
tab_Cp24_bp <- enrich_topgo_to_table(topgo_Cp24)

tab_Cp48_bp <- enrich_topgo_to_table(topgo_Cp48)

tab_Np24_bp <- enrich_topgo_to_table(topgo_Np24)

tab_Np48_bp <- enrich_topgo_to_table(topgo_Np48)

tab_up_Cp24_bp <- enrich_topgo_to_table(topgo_up_Cp24)
tab_up_Cp48_bp <- enrich_topgo_to_table(topgo_up_Cp48)
tab_up_N24_bp <- enrich_topgo_to_table(topgo_up_Np24)
tab_up_N48_bp <- enrich_topgo_to_table(topgo_up_Np48)

tab_down_Cp24_bp <- enrich_topgo_to_table(topgo_down_Cp24)
tab_down_Cp48_bp <- enrich_topgo_to_table(topgo_down_Cp48)
tab_down_N24_bp <- enrich_topgo_to_table(topgo_down_Np24)
tab_down_N48_bp <- enrich_topgo_to_table(topgo_down_Np48)
```
...
go_bp_up_graph_C24 <- plot_enrich_bar(tab_up_Cp24_bp)
go_bp_down_graph_C24 <- plot_enrich_bar(tab_down_Cp24_bp)

go_bp_up_graph_C48 <- plot_enrich_bar(tab_up_Cp48_bp)
go_bp_down_graph_C48 <- plot_enrich_bar(tab_down_Cp48_bp)

go_bp_up_graph_N24 <- plot_enrich_bar(tab_up_N24_bp)
go_bp_down_graph_N24 <- plot_enrich_bar(tab_down_N24_bp)

go_bp_up_graph_N48 <- plot_enrich_bar(tab_up_N48_bp)
go_bp_down_graph_N48 <- plot_enrich_bar(tab_down_N48_bp)
...
```{r}
write.table(tab_Cp24_bp, 
            file = "enriched_Cp24_go_terms.tsv", 
            row.names = FALSE,
            quote = FALSE, 
            sep = "\t")

write.table(tab_Cp48_bp, 
            file = "enriched_Cp48_go_terms.tsv", 
            row.names = FALSE,
            quote = FALSE, 
            sep = "\t")

write.table(tab_Np24_bp, 
            file = "enriched_Np24_go_terms.tsv",
            row.names = FALSE,
            quote = FALSE, 
            sep = "\t")
write.table(tab_Np48_bp, 
            file = "enriched_Np48_go_terms.tsv", 
            row.names = FALSE, 
            quote = FALSE, 
            sep = "\t")
...
temp_df <- top_pval
temp_df$query <- rownames(top_pval)

genes_to_annotate <- semi_join(go_terms, temp_df, "query")
write.table(genes_to_annotate, 
            file = "gos_to_annotate.tsv", 
            sep = "\t", 
            quote = FALSE, 
            row.names = FALSE)

# The go terms were pulled with API requests so they are in JSON format
go_json <- fromJSON("gos_of_interest.json")
go_meta <- fromJSON("data.json")

# Simple pattern matching for GO annotations, only works because I have prior info about
# The terms I am interest in
go_category_rules <- c(
  Defense = "defense|fungus|bacterium|bacteria|chitin|antibiotic",
  Hormone = "hormone|jasmonic|salicylic|gibberellin",
  Development = "seed development|root hair|morphogenesis|cell wall|leaf senescence",
  Stress = "hypoxia|ozone|nitrogen starvation|oxidant"
)
```

```{r}
save_heatmap <- function(to_pdf, ht, file_name) {
  if (!to_pdf) {
...
# A helper function to clean up the go term meta data
clean_json_values <- function(x) {
...
genes <- rownames(m)
primary_category <- setNames(rep("Other", length(genes)), genes)
has_defense <- setNames(rep("No", length(genes)), genes)
has_hormone <- setNames(rep("No", length(genes)), genes)
has_development <- setNames(rep("No", length(genes)), genes)
has_stress <- setNames(rep("No", length(genes)), genes)

priority <- c("Defense", "Hormone", "Development", "Stress", "Other")

go_to_category <- setNames(rep("Other", length(go_meta)), names(go_meta))
for (go_id in names(go_meta)) {
  label <- tolower(go_meta[[go_id]]$label %||% "")
  hits <- names(go_category_rules)[vapply(
    go_category_rules,
    function(pat) grepl(pat, label, perl = TRUE),
    logical(1)
  )]
  if (length(hits) > 0) {
    go_to_category[go_id] <- hits[1]
  }
}

for (g in genes) {
  go_ids <- clean_json_values(go_json[[g]] %||% character(0))
  if (length(go_ids) == 0) {
    next
  }

  categories <- go_to_category[go_ids]
  categories <- categories[!is.na(categories)]
  if (length(categories) == 0) {
    categories <- "Other"
  }

  category_count <- sort(table(categories), decreasing = TRUE)
  tied <- names(category_count)[category_count == max(category_count)]
  picked <- priority[priority %in% tied][1]
  primary_category[g] <- picked

  if ("Defense" %in% categories) has_defense[g] <- "Yes"
  if ("Hormone" %in% categories) has_hormone[g] <- "Yes"
  if ("Development" %in% categories) has_development[g] <- "Yes"
  if ("Stress" %in% categories) has_stress[g] <- "Yes"
}

# Trying complex heatmap instead
treatments <- annotation_col$Condition
col_labels <- structure(treatments, names = rownames(annotation_col))
...
if (!is.null(rownames(m))) {
  row_group[rownames(m) %in% genes_of_interest] <- "Defense Response"
}

# right_ann <- rowAnnotation(
#   Enriched = row_group,
#   show_annotation_name = FALSE,
#   col = list(Enriched = c("Defense Response" = "#E64B35", "Other" = "grey"))
# )

right_ann <- rowAnnotation(
  Defense = has_defense[genes],
  Hormone = has_hormone[genes],
  Development = has_development[genes],
  Stress = has_stress[genes],
  show_annotation_name = FALSE,
  col = list(
    Defense = c(Yes = "#CB4B16", No = "#DADADA"),
    Hormone = c(Yes = "#2AA198", No = "#DADADA"),
    Development = c(Yes = "#268BD2", No = "#DADADA"),
    Stress = c(Yes = "#fc0384", No = "#DADADA")
  )
)
...
save_heatmap(FALSE, map, "results/heatmap.png")
```
