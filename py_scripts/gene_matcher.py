...
def main(blast_file, gene_file, blast_index, gene_index, output_file):
    ...
    blast_df = pd.read_csv(
        blast_file, sep="\t", header=None, index_col=False, low_memory=False
    )
    ...
    matched = blast_df[
        blast_df[blast_column].astype(str).str.strip().isin(upregulated_genes)
    ]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Adds blast terms to a file with diff expressed genes."
    )
    ...
    parser.add_argument(
        "output_file",
        help="Output file with blast terms matched to diff expressed genes",
    )
    parser.add_argument(
        "--ID1", dest="blast_index", required=True, help="Index containing gene IDs"
    )
    parser.add_argument(
        "--ID2", dest="gene_index", required=True, help="Index containing gene IDs"
    )
