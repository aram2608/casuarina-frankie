...
for sam_file in $sam_file_directory/*.sam; do
    ...
    #samtool conversion
    gunzip $sam_file | samtools view -@ 20 -Sb $output_bam $sam_file | samtools sort -O bam -o $sorted_bam $output_bam
    ...
    if [ $PIPESTATUS[0] -ne 0 ] || [ $PIPESTATUS[1] -ne 0 ] || [ $PIPESTATUS[3] ]; then #checks for command success and logs failures
        ...
        echo "Pipe Failure: $PIPESTATUS[@]" >> failurelog.txt
        continue
    fi
    ...
    echo beginning BRAKER2 analysis for gene structure annotation

    #braker2 analysis
    for genome in $genome_directory\*.fna;do
    braker.pl --genome $genome --bam $output_bam --softmasking --cores 25 --species $species 
done
